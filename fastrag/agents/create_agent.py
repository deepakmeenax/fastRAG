...
def get_basic_conversation_pipeline(args):
    ...
    if "system_tools" in conversation_config:
        for tool_config in conversation_config["system_tools"]:
            tool_type = tool_config["type"]
            tool_type_class = TOOLS_FACTORY[tool_type]
...
def get_agent_conversation_pipeline(args, custom_instructions=None, additional_context=None):
    ...
    if additional_context:
        AGENT_CONVERSATION_BASE_ROLES[0]["content"] = AGENT_CONVERSATION_BASE_ROLES[0]["content"].format(
            query="{query}",
